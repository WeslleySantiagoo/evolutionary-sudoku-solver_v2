...
class SudokuGUI(Frame):
    ...
    def __init__(self, master, file):
        ...
        if master:
            master.title("SOLUCIONADOR DE SUDOKU")
        ...
        Label(frame_desafio, text="QUEBRA-CABEÇA", font=("Arial", 10)).pack(expand=True)
        ...
        Label(frame_melhor_individuo, text="ALGORITMO GENÉTICO", font=("Arial", 10)).pack(expand=True)
        ...
        self.generation_label = Label(self.main_frame, text="Geração Atual: N/A", font=("Arial", 10))
        self.generation_label.pack()
        self.total_generation_label = Label(self.main_frame, text="Total de Gerações: N/A", font=("Arial", 10))
        self.total_generation_label.pack()
        ...
        try:
            while True:
                ...
                if update_type == 'status_message':
                    ...
                    gen_num = update_data['generation_num']
                    if isinstance(gen_num, int):
                        self.generation_label.config(text=f"Geração Atual: {gen_num}")
                        self.total_generation_label.config(text=f"Total de Gerações: {gen_num + 1}")
                    else:
                        self.generation_label.config(text=f"Geração Atual: {gen_num}")
                        self.total_generation_label.config(text=f"Total de Gerações: {gen_num}")
                    
                    self.individuals_label.config(text=f"Indivíduos Gerados: {update_data['total_individuals']}")
                    best_fitness_val = update_data['best_fitness']
                    if isinstance(best_fitness_val, (int, float)):
                        self.best_fitness_label.config(text=f"Melhor Aptidão: {best_fitness_val:.6f}")
                    else:
                        self.best_fitness_label.config(text=f"Melhor Aptidão: {best_fitness_val}")
                    self.update_canvas_3(update_data['best_candidate_values'])
                elif update_type == 'time_update':
                    self.elapsed_time_label.config(text=f"Tempo Decorrido: {update_data['elapsed_time']:.4f}s")
    ...
    def start_solver_w_pp(self):
        ...
        if not np.any(self.sudoku_pre_processed == 0):
            ...
            self.update_queue.put({'type': 'ga_progress', 'generation_num': "N/A", 'best_candidate_values': np.zeros((9,9)).astype(int), 'total_individuals': "N/A", 'best_fitness': "N/A"})        
            return
        
        self.start_solver(use_preprocessing_result=True)
    ...
    def start_solver_wo_pp(self):
        ...
        self.start_solver(use_preprocessing_result=False)
    ...
    def _run_solver(self, use_preprocessing_result):
        ...
        if generation == -1:
            ...
            str_print = "SOLUÇÃO ENCONTRADA COM O PRÉ-PROCESSAMENTO + ALGORITMO GENÉTICO"
            self.is_solution_complete = True
        else:
            str_print = "SOLUÇÃO NÃO ENCONTRADA"
    ...
    def update_canvas_2(self, board=None):
            ...
            is_preprocessed_solution_complete = not np.any(g == 0)
                ...
                for x in range(9):
                    if g[y][x] != 0:
                        if self.original_sudoku_problem[y][x] != 0:
                            fill_color = "black"
                        elif is_preprocessed_solution_complete:
                            fill_color = "green"
                        else:
                            fill_color = "red"
        ...
        for y in range(9):
            for x in range(9):
                if board[y][x] != 0:
                    if self.original_sudoku_problem[y][x] != 0:
                        fill_color = "black" 
                    elif self.is_solution_complete:
                        fill_color = "green"
